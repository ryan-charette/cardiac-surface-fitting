...
def bench_tspline(case, grid, repeats):
    ...
    rows.append(
        [
            "tspline_eval",
            "torch_framework_cuda",
            case,
            f"{grid[0]}x{grid[1]}",
            samples,
            mean_ms,
            median_ms,
            samples / (mean_ms / 1000),
            err,
        ]
    )
    ...
    try:
        ...
        rows.append(
            [
                "tspline_eval",
                f"triton_failed:{type(exc).__name__}",
                case,
                f"{grid[0]}x{grid[1]}",
                samples,
                "",
...
def main():
    ...
    parser.add_argument("--case", default="bifurcation", choices=["tube", "bifurcation"])
    parser.add_argument("--grid", nargs=2, type=int, default=[400, 200])
    parser.add_argument("--chamfer-points", type=int, default=8192)
    parser.add_argument("--repeats", type=int, default=20)
    parser.add_argument("--output", type=Path, default=Path("outputs/gpu_benchmark_4090.csv"))
